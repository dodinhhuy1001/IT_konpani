              ...
              <div class="col-sm-12">
                <div class="panel panel-default">
                    <div class="panel-body"> 
                        ...
                        <?php echo form_open('Cmanufacturer/manufacturer_sales_details_datewise', array('class' => 'form-inline', 'method' => 'get')) ?>
                        <?php
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="from_date" class="form-control datepicker" id="from_date" value="<?php echo  (!empty($fromdate)?html_escape($fromdate):date('Y-m-d'))?>" placeholder="<?php echo display('start_date') ?>" >
                        </div> 

                        <div class="form-group">
                            <label class="" for="to_date"><?php echo display('end_date') ?></label>
                            <input type="text" name="to_date" class="form-control datepicker" id="to_date" placeholder="<?php echo display('end_date') ?>" value="<?php echo  (!empty($todate)?html_escape($todate):date('Y-m-d'))?>">
                        </div>  
			                    ...
			                    <table class="table table-bordered table-striped table-hover">
									...
									{sales_info}
										<tr>
											...
											<td align="right">{manufacturer_name}</td>
											<td align="center"><a href="<?php echo base_url().'Cinvoice/invoice_inserted_data/{invoice_id}'; ?>">{invoice}</a></td>
											<td align="right">{quantity}</td>
											<td class="text-right !Important"><?php echo (($position==0)?"$currency {manufacturer_rate}":"{manufacturer_rate} $currency") ?></td>
