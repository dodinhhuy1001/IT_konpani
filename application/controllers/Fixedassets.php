...
class Fixedassets extends CI_Controller {
 ...
 $this->session->set_userdata(array('error_message' => display('item_code_already_exists')));
        ...
        if ($result == 1) {
            ...
            $stockassets   = array(
            'item_code'      => $this->input->post('item_code',true),
            'qty'            => $this->input->post('previous_stock',true),
            );
            if(!empty($this->input->post('previous_stock',true))){
    ...
    public function fixed_asset_update() {
        ...
        $item_code = $this->input->post('old_item_code',true);
        $id        = $this->input->post('id',true);
        $n_item_code = $this->input->post('item_code',true);
        $data = array(
            'item_code'      => $this->input->post('item_code',true),
            'item_name'      => $this->input->post('item_name',true),
            'price'          => $this->input->post('price',true),
    ...
    public function delete_assets($item_code) {
        ...
        if ($result) {
            ...
            return redirect("Fixedassets/fixed_assets_list");
        }
    }
 ...
 /// Fixed Assets purchase from supplier
    ...
    // Fixed assets information search
     ...
     public function retrieve_asset_data() {
        ...
        $this->auth->check_admin_auth();
        $CI->load->model('Fixedasset');
        $asset_item_code = $this->input->post('asset_item_code',true);
   ...
   public function update_asserts_purchase() {
    ...
    public function purchase_pad_print($purchase_id) {
     ...
     public function purchase_print($purchase_id) {
        ...
        $CI->auth->check_admin_auth();
        $CI->load->library('lfixedasset');
        $content = $CI->lfixedasset->purchase_print($purchase_id);
        $this->template->full_admin_html_view($content);
    }
